...
@router.get("/users/{user_email}/servers/{server_id}/tool-permissions")
async def list_user_tool_permissions(
    user_email: str,
    server_id: str,
    admin: dict = Depends(require_admin),
    db: AsyncSession = Depends(get_db),
    ...
    """
    List all tools for a server with user-level permissions.
    ...
    **Enterprise Edition Feature**: Per-user tool permissions are only available
    ...
    Raises:
        HTTPException 402: Enterprise feature required
    """
    raise HTTPException(
        status_code=402,
...
@router.put("/users/{user_email}/tools/{tool_id}/toggle")
async def toggle_user_tool_permission(
    user_email: str,
    tool_id: str,
    enabled: bool,
    admin: dict = Depends(require_admin),
    ...
    """
    Set user-level override for a specific tool.
...
@router.delete("/users/{user_email}/tools/{tool_id}/override")
async def remove_user_tool_permission_override(
    user_email: str,
    tool_id: str,
    admin: dict = Depends(require_admin),
    db: AsyncSession = Depends(get_db),
    ...
    """
    Remove user-level override, reverting to organization-level default.
...
@router.post("/users/{user_email}/tools/bulk-toggle")
async def bulk_toggle_user_tool_permissions(
    user_email: str,
    request: BulkToggleRequest,
    admin: dict = Depends(require_admin),
    db: AsyncSession = Depends(get_db),
    ...
    """
    Enable or disable multiple tools for a user.
